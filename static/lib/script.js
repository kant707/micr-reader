...
(function() {
    var f = $("#ic-cropper-wrap"),
        h = $("#ic-info"),
        b, d, imgInfo, imgHeight, imgWidth;
    ...
    $d.on("click", "#ic-download-btn", function() {
        ...
        //     for (var e = c, c = 0 <= e.split(",")[0].indexOf("base64") ? atob(e.split(",")[1]) : unescape(e.split(",")[1]), e = e.split(",")[0].split(":")[1].split(";")[0], f = new Uint8Array(c.length), g = 0; g < c.length; g++) f[g] = c.charCodeAt(g);
        //     c = new Blob([f], {
        //         type: e
        //     });
        //     c = URL.createObjectURL(c)
        // }
        const micrStringProcessing = (str) => {
            const removingJunk = str.split(/\s|\n/);
            const filteredArray = removingJunk.filter(Boolean);
            // const ck = filteredArray;
            const separatedString = filteredArray;
            const routingNumber = separatedString[0].replace(/\D/g,'');
            const accountNumber = separatedString[1].replace(/\D/g,'');

            const routingNumberHolder = document.querySelector('.routing-number');
            const accountNumberHolder = document.querySelector('.account-number');
            routingNumberHolder.innerText = routingNumber;
            accountNumberHolder.innerText = accountNumber;
        }

        const resetter = () => {
            const inputFile = document.getElementById('fileInput');
            inputFile.value = '';
            document.getElementById('ic-cropper-wrap').innerHTML = '';
            $(".ic-crop-btn-wrap, .ic-result-wrap, .ic-download-wrap").addClass("ic-hidden");
            $(".cropper").hide();
        }
        ...
        const sendData = async () => {
            try {
                ...
                const config = {
                    method: "POST",
                    body: JSON.stringify({ image: imgInfo, image_height: imgHeight, image_width: imgWidth }),
                    headers: headers
                };

                const response = await fetch("/upload", config).then((res) => res.json());
                micrStringProcessing(response.micr_text);
                resetter();
